...
class TestWebSocketAuth:
    ...
    async def test_connect_with_valid_token(self, db_session, user):
        ...
        token = create_access_token("admin@example.com", FAKE_USER_ID, timedelta(minutes=5))
...
class TestGetLocksEndpoint:
    @pytest.mark.asyncio
    async def test_get_locks_empty(self, db_session, user, lock_server_client, client):
        response = await client.get("/system/locks")
        assert response.status_code == 200
        assert response.json() == []

    @pytest.mark.asyncio
    async def test_get_locks_with_active_lock(self, db_session, user, lock_server_client, client):

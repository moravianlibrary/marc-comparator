...
class TestCookieAuth:
    ...
    @pytest.mark.asyncio
    async def test_login_wrong_password_returns_401(self, db_session, client: AsyncClient):
        Role.create_default_roles(db_session)
        await client.post(
            "/auth/sign-up",
            json={
...
class TestRefreshToken:
    ...
    @pytest.mark.asyncio
    async def test_refresh_without_token_returns_401(self, db_session, client: AsyncClient):
        response = await client.post("/auth/refresh")
        assert response.status_code == 401
...
class TestOIDCAuth:
    @pytest.mark.asyncio
    async def test_oidc_login_redirects_when_enabled(self, db_session, client: AsyncClient, mocker):
    ...
    @pytest.mark.asyncio
    async def test_oidc_login_returns_404_when_disabled(self, db_session, client: AsyncClient):

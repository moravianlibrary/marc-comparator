...
class RuleBasedComparator(BaseComparator):
    ...
    async def run(
        ...
        return RecordComparisonResult(
            overall_score=overall_score, field_results=results
        )
...
def compare_values(
    ...
    if value_a == value_b:
        return SubfieldComparisonResult(
            code=code, score=field_rule.match_score
        )
...
def compare_variable_fields(
    ...
    scores: List[float] = []
    results: List[SubfieldComparisonResult] = []
    ...
    for code in subfield_codes:
        ...
        matched_results: List[SubfieldComparisonResult] = []
...
def compare_records(
    ...
    used_a, used_b = set(), set()
    matched_scores: List[float] = []
    matched_results: List[FieldComparisonResult] = []

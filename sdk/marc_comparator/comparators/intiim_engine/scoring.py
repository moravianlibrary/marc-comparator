...
from .config import RECORD_CAP, AGGREGATION, AGG_P, HINGE_ACCEL_ENABLED, HINGE_POINT, HINGE_EXP
...
EXTRA_REASONS = {"extra_in_test", "extra_heading_in_test", "extra_occurrence_in_test", "auxiliary_added"}
MISSING_REASONS = {
    "missing_in_test",
    "missing_in_main",
...
def _load_weights(path: Optional[str] = None) -> Dict[str, int]:
...
def _weight_key(tag: str, code: Optional[str], category: str) -> Tuple[str, ...]:
    keys = []
    if code:
...
def _pick_weight(weights: Dict[str, int], tag: str, code: Optional[str], category: str, default: int = 1) -> int:
    for k in _weight_key(tag, code, category):
        if k in weights:
            return int(weights[k])
    return default

def _field_overall_cap(weights: Dict[str, int], tag: str) -> Optional[int]:
...
def score_differences(differences: List[Dict[str, Any]], *, weights_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    for d in differences:
        ...
        entry = {
            "tag": tag, "code": code, "label": d.get("label"),
            "category": category, "weight": w,
    ...
    accelerated = (
        _hinge_accelerate(base, hinge=float(HINGE_POINT), cap=float(RECORD_CAP), exp=float(HINGE_EXP))
        if HINGE_ACCEL_ENABLED else base
    )

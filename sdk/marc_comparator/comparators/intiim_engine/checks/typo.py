...
from ..config import ASK_LLM_IF_UNSURE, SIM_HIGH, SIM_MED, TOKEN_JACCARD_MED
    ...
    if SIM_MED <= sim < SIM_HIGH and jac >= TOKEN_JACCARD_MED:
        if ASK_LLM_IF_UNSURE and allow_llm:
            try:
                ...
                review = verify_label(
                    a, b, role, label="TYPO", tag="(n/a)", code=None, backend=backend
                )
                ...
                logger.warning(
                    "LLM verification failed in typo check, defaulting to TYPO with reduced confidence",
                    exc_info=True,
                )
                return {
                    "label": "TYPO",
                    "confidence": 0.6,
                    "details": {"sim": sim, "jaccard": jac, "llm": "error"},
                }
